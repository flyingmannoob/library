      ...
      <div class="main-content">
        ...
        <div v-else>
          <ul class="book-list">
            ...
            class="book-item"
          >
              <div class="book-content" @click="goToBookDetail(book.id)">
              ...
              <div class="book-likes">
                ...
                <span 
                  :class="['heart-icon', book.isLiked ? 'liked' : '']"
...
export default {
  ...
  data() {
    return {
      ...
      chart: null,
      likedBooks: new Set() // 存储用户点赞过的图书ID
    };
  },
  computed: {
    filteredBooks() {
  ...
  methods: {
    async fetchBooks() {
      ...
      try {
        ...
        this.books = booksData.map(book => ({
          ...book,
          isLiked: this.likedBooks.has(book.id) // 根据本地存储初始化状态
        }));
      } catch (err) {
        ...
        // 使用模拟数据
        this.books = this.getMockData().map(book => ({
          ...book,
          isLiked: this.likedBooks.has(book.id)
        }));
      } finally {
    ...
    async toggleLike(book) {
      try {
        const bookId = book.id;
        const currentLikes = book.likes;
        const currentIsLiked = book.isLiked;
        ...
        const apiUrl = newIsLiked ? 
          ...
          `http://localhost:8080/api/books/${bookId}/unlike`;
        ...
        if (!response.ok) {
          ...
          console.error(newIsLiked ? '点赞失败' : '取消点赞失败');
        }
      } catch (error) {
    ...
    // 安全更新书籍列表
    updateBookInList(bookId, updates) {
